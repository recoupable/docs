...
**GET**, **POST**, **PATCH**, and **DELETE** on `/api/tasks` all require authentication. Use the same **exactly-one** rule: **`x-api-key`** or **`Authorization: Bearer`**. For **PATCH** and **DELETE**, the server checks that the caller may access the task owner's account before applying the change. **PATCH** does not apply `account_id` from the body for reassignment (omit it). Details: [Get Tasks](/api-reference/tasks/get), [Create](/api-reference/tasks/create), [Update](/api-reference/tasks/update), [Delete](/api-reference/tasks/delete).
