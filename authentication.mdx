...
<Info>
Org membership is determined by the `account_organization_ids` table. An account gains access to an org when it is added as a member.
</Info>
...
Membership is verified by querying the `account_organization_ids` table for a record linking the key owner's account to the target account's organization.
