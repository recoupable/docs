...
## Chat App Fallback Endpoint Security

The Recoup web app includes an internal fallback endpoint:

- `DELETE https://chat.recoupable.com/api/scheduled-actions/delete`

This route exists for fallback cleanup when scheduler deletion cannot be completed through the primary API path.

### Required Access Rules

- Caller must be authenticated (valid session/token).
- Caller must be authorized to delete the specific task ID.
- Deletion must be identity-bound (ownership derived from authenticated identity, not request body fields).

<Info>
Primary task deletion should continue to use `DELETE /api/tasks`. The chat fallback endpoint is only for fallback DB cleanup paths.
</Info>
...
| `403` | Authenticated caller is not allowed to delete this task |
| `404` | Task ID not found |

### Billing Endpoint Contract (`POST /api/stripe/session/create`)

For chat billing flows, `POST https://chat.recoupable.com/api/stripe/session/create` must follow identity-bound authorization:

- Caller must be authenticated (Bearer/session).
- The server must derive the target account from authenticated identity.
- If an account override is supported, it must be explicitly authorized (org/admin policy).
- Do not trust client-provided `accountId` as ownership proof.
...
| `400` | Invalid/missing input (e.g., `successUrl`) |
| `401` | Missing or invalid authentication |
...
```bash
curl -X DELETE "https://chat.recoupable.com/api/scheduled-actions/delete" \
  -H "Authorization: Bearer YOUR_PRIVY_JWT" \
  -H "Content-Type: application/json" \
  -d '{"id":"UUID_OF_SCHEDULED_ACTION"}'
```
