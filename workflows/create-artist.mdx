...
**After this step:** populate the artist's `releases/` folder — write one `releases/{album-slug}/RELEASE.md` per album (kebab-case the album name; suffix `-single` / `-ep` to disambiguate when needed) using the per-album `GET /api/spotify/album?id=$ALBUM_ID` response, and write the top tracks snapshot to `releases/top-tracks.md`. The per-album `RELEASE.md` structure (frontmatter + tracklist + Notes) and the top-tracks template are defined in the [`artist-workspace`](https://github.com/recoupable/skills/tree/main/skills/artist-workspace) skill — see its **Releases** section + `references/release-template.md`. Tick `- [ ] 5.` → `- [x] 5.`.
