...
Once the MP4 is rendered, push it to the artist's socials with the [Connectors API](/api-reference/connectors/list). Heads-up gotchas worth knowing before you ship:
...
- **Unaudited app.** If the connected TikTok dev app is in sandbox mode, publishing returns `unaudited_client_can_only_post_to_private_accounts` regardless of `privacy_level`. The connected handle must be added as a Test User in the TikTok dev portal.
- **Drafts path.** `publish=false` puts the video in TikTok drafts for the user to finalize in-app — but requires the `video.upload` OAuth scope, which isn't granted by default. Re-auth with that scope, or use `publish=true`.
