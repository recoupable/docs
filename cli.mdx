...
| Flag | Required | Description |
|------|----------|-------------|
| `--audio <url>` | Yes | Public URL to the audio file (MP3, WAV, FLAC) |
| `--preset <name>` | No | Curated analysis preset (omit when using `--prompt`) |
| `--prompt <text>` | No | Custom text prompt (omit when using `--preset`) |
