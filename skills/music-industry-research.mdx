...
A structured research output answering the user's question — usually a synthesis across multiple `/api/research/*` calls (e.g. profile + metrics + audience + similar artists), formatted for either the agent's continued reasoning or direct delivery to the user.

## Endpoints it uses

All of [Research](/api-reference/research/search). The skill's decision tree picks the minimal set per query.
