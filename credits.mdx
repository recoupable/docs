...
| Plan | Price | Monthly credits | Scheduled tasks | Shortest cadence | `plan` value |
|------|-------|-----------------|-----------------|------------------|--------------|
| **Free** | $0 | $3.33 | 1 | Weekly (10080 minutes) | `free` |
| **Starter** | $19/mo | $20 | 3 | Daily (1440 minutes) | `starter` |
| **Pro** | $99/mo, 30-day trial | $300 | Unlimited | Hourly (60 minutes) | `pro` |

The plan is resolved from the account's active Stripe subscription price, never stored: the Starter price maps to `starter`; any other active subscription, an organization subscription, or an enterprise email domain maps to `pro`; no subscription is `free`. Roster-wide social monitoring, emailing reports to recipients other than yourself, and API keys stay Pro-only, so `is_pro` is false on Starter.

### Task limits

[`POST /api/tasks`](/api-reference/tasks/create) and [`PATCH /api/tasks`](/api-reference/tasks/update) check the plan before writing. Creating a task past `task_limit`, re-enabling a disabled task past it, or saving a cron whose consecutive runs are closer together than `min_cadence_minutes` returns **HTTP 402** with `error: "plan_limit"`:
...
`limit` is `task_count` or `min_cadence`; `current_task_count` excludes the task being created or updated. Edits that only touch `title`, `prompt`, or `model` never hit the gate. Tasks that existed before task limits shipped keep running until 2026-10-01 so their owners can pick a plan; after that date a task outside its plan's limits is skipped instead of run.
...
1. Sign in at [app.recoupable.dev](https://app.recoupable.dev)
2. Open the account menu and pick a plan, or call [`POST /api/subscriptions/sessions`](/api-reference/subscriptions/sessions-create) with `plan` set to `starter` or `pro` and open the returned checkout `url`
