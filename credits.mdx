...
| **Music analysis** ([`POST /api/songs/analyze`](/api-reference/songs/analyze)) | Yes | 2× the model's measured compute time (`max($0.01, 2 × elapsed_seconds × $0.000583)` per model call); `full_report` is the sum of its 13 calls. |
...
| [`POST /api/songs/analyze`](/api-reference/songs/analyze) | `max($0.01, 2 × elapsed_seconds × $0.000583)` per model call; `full_report` = 13 calls |
