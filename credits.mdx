...
| **Music analysis** ([`POST /api/songs/analyze`](/api-reference/songs/analyze)) | Yes | \$0.01 per model call plus \$0.001166 per second of inference time; `full_report` is the sum of its 13 calls. |
...
| [`POST /api/songs/analyze`](/api-reference/songs/analyze) | `$0.01 + $0.001166 × elapsed_seconds` per model call; `full_report` = 13 calls |
