...
| **Music analysis** ([`POST /api/songs/analyze`](/api-reference/songs/analyze)) | Yes | \$0.01 per model call plus a per-second inference rate (currently \$0.001166 per second, tracking the GPU price we pay); `full_report` is the sum of its 13 calls. |
...
| [`POST /api/songs/analyze`](/api-reference/songs/analyze) | `$0.01 + rate × elapsed_seconds` per model call, `rate` currently \$0.001166 per second; `full_report` = 13 calls |
