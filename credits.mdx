...
Before triggering a silent off-session charge, inspect the default Stripe payment method on file:
...
`card` is `null` when no payment method has been saved yet — the next top-up call will route through Stripe Checkout to collect one. Expired cards are still returned (with their original `exp_month` / `exp_year`); callers should compare against the current date and warn the user, since an off-session charge against an expired card will decline. Full schema at [Get Default Payment Method](/api-reference/accounts/payment-method-get).
