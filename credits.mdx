...
If you read `checkoutUrl`, read [`billingUrl`](#402-payment-required) instead for a link to send a human to, or call [`POST /api/credits/sessions`](/api-reference/credits/sessions-create) to mint a real Checkout URL on demand. If you read `declineReason`, it is still returned by `POST /api/credits/sessions` on the consented top-up path.
</Note>
