...
| **Music analysis** ([`POST /api/songs/analyze`](/api-reference/songs/analyze)) | Yes | \$0.05 per model call plus \$0.001166 per second of inference time; `full_report` is the sum of its 13 calls. Free accounts can analyze 5 tracks a month ([analyze limits](#analyze-limits)). |
...
Recoup has three plans. All three refill credits on a monthly cycle, and the plan also sets how many scheduled tasks an account can run, how often they can fire, and how many tracks it can analyze a month.
...
| **Tracks analyzed a month** | 5 | Unlimited | Unlimited |
...
[`POST /api/songs/analyze`](/api-reference/songs/analyze) checks the plan before the credit balance and before the model runs. Free accounts can analyze **5 tracks per calendar month** (UTC); Starter and Pro accounts are uncapped (`analyze_limit: null`). A track is counted once by its `audio_url`, so `full_report` counts as one track and analyzing the same track again in the same month costs credits but does not use another slot. The sixth new track returns **HTTP 402** with `error: "plan_limit"`:
...
Nothing is charged and no Modal container starts. The `analyze_music` MCP tool returns the same `message` as a tool error. The count resets on the first of the month; an upgrade lifts it immediately.
...
1. Sign in at [app.recoupable.dev/plan](https://app.recoupable.dev/plan)
2. Pick a plan, or call [`POST /api/subscriptions/sessions`](/api-reference/subscriptions/sessions-create) with `plan` set to `starter` or `pro` and open the returned checkout `url`
