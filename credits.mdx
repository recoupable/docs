...
The plan is resolved from the account's active Stripe subscription price, never stored: the Starter price maps to `starter`; any other active subscription, an organization subscription, or an enterprise email domain maps to `pro`; no subscription is `free`. Roster-wide social monitoring and emailing reports to recipients other than yourself stay Pro-only, so `is_pro` is false on Starter.
