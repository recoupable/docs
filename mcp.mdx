...
Recoup exposes two MCP servers: one for **searching these docs**, and one for **calling Recoup tools** from your AI agent.
...
## 2. Recoup API MCP Server

The Recoup API exposes an MCP server that AI agents can connect to for tool use. The server is available at:
...
Get your key from the [API Keys page](https://chat.recoupable.com/keys). All tools require your key as a Bearer token.

---

### Connect with Claude Desktop

Add this to your `claude_desktop_config.json` (found at `~/Library/Application Support/Claude/` on macOS):
...
Add this to `.cursor/mcp.json` in your project root, or to `~/.cursor/mcp.json` globally:
...
### Connect programmatically (TypeScript SDK)
...
```typescript
// Run a prompt in a persistent sandbox
const result = await client.callTool({
  name: "prompt_sandbox",
